...
def trainVideo2(file,label,filename,bboxes):
    ...
    corners=[]
    img=[]
    storepath = 'ib30-original/'+str(label)+"/"
    nos = 100
    if not os.path.exists(storepath):
        os.mkdir(storepath)
    fname = filename[:-len('.mp4')]
    storepath = storepath + fname
    if not os.path.exists(storepath):
        os.mkdir(storepath)
...
# #                 out.write(padded_img)
        ...
        # fno=fno+1
        # cv2.imwrite(storepath+'/'+fname+'_'+str(fno)+'.png',frame)
...
#     box = boxes[allfiles[0]][221]
    ...
    # for file in ["wyND5beWDCU_full_corr.mp4"]:
        if file.endswith(".mp4"):
            ...
            # svc.fit(X,Y)
#             if(j==1):
            trainVideo2(fullPath+file,i,file,boxes[file])
        #     j=j+1
        # if (j==1):
        #     break
