...
def loadFrameNo(f1,no):
    img = cv2.imread(loadpath+f1+'-'+str(no)+'-frame.png');
    im_max = max(img.flatten())
    im_min = min(img.flatten())
    img = (img - im_min)/(im_max - im_min)
    sil = cv2.imread(loadpath+f1+'-'+str(no)+'-gt.png');
    return img,sil
...
def trainVideo2(file,boxes,label,filename,frameno):
    ...
    corners=[]
    img=[]
    storepath = '/home/imgprocessgrp/krunal/code/output/'+str(label)+"/"
    nos = 100
    if not os.path.exists(storepath):
        os.mkdir(storepath)
    fname = filename.strip('.mp4')
    writer = skvideo.io.FFmpegWriter(storepath+fname+"_full_corr.mp4")
    ...
    while True:
        ...
        if fno in boxes:
            ...
            print("Frame:"+str(fno));
            if(fno==keys[frameno]):
                img,sil = loadFrameNo(fname,frameno)
...
#    plt.imshow(reconstructed),plt.show();
    ...
    ret,thresh1 = cv2.threshold(reconstructed,0.4,1,cv2.THRESH_BINARY)
...
basePath = "/home/imgprocessgrp/krunal/IndianBirds/"
frame_array=[]
start_time = datetime.datetime.now();

arr = {11 :["_4HSoT3Jxqc.mp4", "mXQQkeHwqcs.mp4"],
12:["LfoZ7WTSkoU.mp4", "9IMlpMU2oJM.mp4"]}
...
#   for file in allfiles:
    ...
    for file in ['QY5r7cze8Mg.mp4']:
        if file.endswith(".mp4"):
            ...
            svc.fit(X,Y)
#             if(j==1):
            frame_array = trainVideo2(fullPath+file,boxes[file],i,file,frameno)
            f = open("/home/imgprocessgrp/krunal/code/corrVid.txt","w")
            f.write(str(i)+"/"+file+" done.")
            f.close()
        #     j=j+1
        # if (j==2):
        #     break
